...
%% Preparations
    ...
    stem(100*p_o_less);
    ylabel('probability, %');
    xlim([1 length(p_o_less)]);
    title('Periphery to support vector (I_{O}->I_{S})');
    
    subplot(2,1,2);
    hold on;
    scatter(1:length(M_o_less),M_o_less);
    ylabel('M value');
    xlim([1 length(M_o_less)]);
    ylim(min(M_o_less)+ [0 1.5*( max(M_o_less)-min(M_o_less) )]);
    line([1 length(M_o_less)],[1 1],'Color','r','LineWidth',2);
    text(length(M_o_less)*(3/4),1 + 0.2*(max(M_o_less)-min(M_o_less)),'M = 1','Color','r');
